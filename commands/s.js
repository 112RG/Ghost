	...
	fd.pipe(concat({ encoding: 'buffer' }, data => {
		...
		.then((response) => {
			if (response.data.success === false) {
				...
				ghost.error('Error saving sticker. Check logs')
...
exports.migrate = function() {
	...
	_msg.edit('*Starting migration to ghost v4.1.0, this might take a while depending how many stickers you have. Check the console.*')

	try {

		ghost.db.schema.table(_table, (table) => {
			table.string('url')
		}).then(() => {
			// Done for now
			ghost.db.table(_table).then((rows) => {
						...
						fd.pipe(concat({ encoding: 'buffer' }, data => {
							...
							.then((response) => {
								if (response.data.success === false) {
									...
									ghost.db.table(_table).where('id', row.id).update(
									...
									.then(() => {
										...
										if (counter > rows.length) {
											console.log('Migration finished, you can keep using ghost like you normally would')
											_msg.edit('*Migration finished, you can keep using ghost like you normally would*')
										}
									})
								}
							})
